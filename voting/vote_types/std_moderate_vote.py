...
#TODO Add the loop to check active user count
class StdModerateVote:
    bot: Bot
    ...
    async def on_react_add(self, emoji: str, msg: discord.Message, user: discord.Member, t: tuple) -> None:
        ...
        if result:
            await self.give_feedback(result, user, choice_indexes.get(emoji, -1), voteID, limit)

        self.count_valid_users(self.ctx)
        print(f"There are {self.active_member_count} active members.")
    ...
    async def give_feedback(self, result, user: discord.Member, index, vid, limit):
        ...
        if isinstance(result, tuple) and result[0] == "clear votes":
            await user.dm_channel.send(f"Poll {vid}: Your votes have been cleared for:\n\t\t" +
                                       '\n\t\t'.join(f"{choice_symbols[i]} **{options[i]}**" for i in result[1]))

        elif result == "added vote":
            await user.dm_channel.send(f"Poll {vid}: Counted your vote for {choice_symbols[index]} **{options[index]}**")
        elif result == "removed vote":
            await user.dm_channel.send(f"Poll {vid}: Removed your vote for {choice_symbols[index]} **{options[index]}**")
            ...
            await user.dm_channel.send(
                ...
                '\n\t\t\t'.join(f"{choice_symbols[i]} **{options[i]}**" for i, _ in voteDB.getUserVotes(vid, user.id))
            )
    ...
    async def create_vote(self, ctx: Context, args, desc=None, type=200, title_pre: str = "Anonymous Poll", idl = [], pollTime=-1) -> None:
        ...
        limit = 1 #args.limit
        num_wins = 1 #args.winners
        self.ctx = ctx
        self.active_member_count = self.count_valid_users(ctx)
    ...
    def vote_summary(self, args):
        return f"Votes: **{'Hidden' if self.remove_reactions else 'Visible'}** Order: **{self.order_text}** Vote Limit: **1** Winners: **{args.winners}**\n"\
    ...
    async def post_vote(self, ctx: Context, vid: int, title: str, desc: str, options: list[str], colour) -> list[discord.Message]:
        ...
        lines = [f"\n{choice_symbols[0]} {options[0]}", f"\n{choice_symbols[1]} {options[1]}"]
    ...
    async def add_reactions(self, messages: list[discord.Message], options: list[str]) -> None:
        ...
        for i0, m in enumerate(messages):
            ...
            for i in range(i0 * 20, limit):
                await m.add_reaction(choice_symbols[i])
    ...
    def make_results(self, vid: int, num_win: int) -> list[Union[discord.File, EmbedData]]:
        ...
        sections = [StdModerateVote.list_results(options, list(range(len(options))), votes)]
        sections.append(("Total valid users", [""], False))

        return sections


    def count_valid_users(self, ctx: Context):
        membersInServer = ctx.guild.members
        onlineMembersInServer = list(filter(lambda x: x.status != 'offline', membersInServer))
        self.active_member_count =  len(onlineMembersInServer)
    ...
    def list_results(options: list[str], order: list[int], votes: dict[int, int], title="Results") -> EmbedData:
        ...
        return title, [start_msg] + [f"{choice_symbols[i]} **{options[i]}**: **{votes[i]}** votes" for i in order], False
    ...
    def top_n_results(n: int, options: list[str], votes: dict[int, int], title=None) -> EmbedData:
        ...
        return StdModerateVote.list_results(options, picked, votes, title)
